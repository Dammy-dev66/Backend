...
async function readCouponState() {
  const blobToken = cleanString(process.env.BLOB_READ_WRITE_TOKEN);
  const blobStoreId = cleanString(process.env.BLOB_STORE_ID);
  const blobReady = Boolean(blobToken || blobStoreId || process.env.VERCEL_OIDC_TOKEN);
  ...
  if (blobReady) {
    try {
      ...
      // Fall back to local/env configuration when blob access isn't available yet.
    }
  }
