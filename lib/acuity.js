...
async function listAppointments({ minDate, maxDate, max = 100, showall = false, email, appointmentTypeID, calendarID } = {}) {
  const query = buildQuery({ minDate, maxDate, max, showall: showall || undefined, email, appointmentTypeID, calendarID, excludeForms: true, direction: "ASC" });
  return acuityRequest(`/appointments?${query}`);
}

async function getAppointment(id) {
  return acuityRequest(`/appointments/${encodeURIComponent(String(id))}`);
}

async function listAvailabilityTimes({ appointmentTypeID, date, calendarID, ignoreAppointmentID } = {}) {
  const query = buildQuery({ appointmentTypeID, date, calendarID, "ignoreAppointmentIDs[]": ignoreAppointmentID });
  return acuityRequest(`/availability/times?${query}`);
}

async function rescheduleAppointment(id, { datetime, calendarID } = {}) {
  return acuityRequest(`/appointments/${encodeURIComponent(String(id))}/reschedule?admin=true&noEmail=true`, {
    method: "PUT",
    body: JSON.stringify({ datetime, ...(calendarID ? { calendarID: Number(calendarID) } : {}) })
  });
}

async function cancelAppointment(id, { cancelNote, noShow = false } = {}) {
  return acuityRequest(`/appointments/${encodeURIComponent(String(id))}/cancel?admin=true&noEmail=true`, {
    method: "PUT",
    body: JSON.stringify({ ...(cancelNote ? { cancelNote } : {}), noShow: Boolean(noShow) })
  });
}
