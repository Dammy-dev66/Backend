...
const { defaultTemplates, interpolateTemplate, readTemplateState } = require("./email-templates");
...
function buildFinHtml({
  ...
  const rows = [
    ...
    ["Student 2", studentName2],
    ["Package code", certificate]
  ].filter(([, value]) => cleanString(value));
  const details = rows.map(([label, value]) =>
    `<tr><td style="padding:6px 12px 6px 0;color:#56616d;vertical-align:top;">${escapeHtml(label)}</td><td style="padding:6px 0;font-weight:700;vertical-align:top;">${escapeHtml(value)}</td></tr>`
  ).join("");

  return `<!doctype html><html><body style="margin:0;background:#fbfaf7;font-family:Arial,sans-serif;color:#111827;"><div style="max-width:640px;margin:0 auto;padding:32px 20px;"><div style="border:1px solid #d8d1c4;background:#fffdf8;padding:28px;"><p style="margin:0 0 10px;font-size:12px;letter-spacing:.18em;font-weight:700;color:#0a3d91;text-transform:uppercase;">Finbar B. Elite Tutoring</p><h1 style="margin:0 0 14px;font-family:Georgia,'Times New Roman',serif;font-weight:400;font-size:30px;line-height:1.15;">${escapeHtml(heading || "Booking notification")}</h1><p style="margin:0 0 18px;font-size:14px;line-height:1.6;color:#56616d;">A customer booking update has been received.</p><div style="border:1px solid #d8d1c4;background:#fff;padding:16px 18px;margin:0 0 18px;"><div style="font-size:15px;font-weight:700;margin-bottom:6px;">${escapeHtml(subject || "Booking")}</div><div style="font-size:13px;line-height:1.6;color:#56616d;">${escapeHtml(format || "")}${format && tier ? " - " : ""}${escapeHtml(tier || "")}</div><div style="font-size:13px;line-height:1.6;color:#56616d;">Total: ${escapeHtml(formatMoney(totalPrice))}</div></div><table role="presentation" style="width:100%;border-collapse:collapse;font-size:13px;line-height:1.45;margin:0 0 18px;">${details}</table><p style="margin:0;"><a href="${escapeHtml(bookingLink)}" style="color:#0a3d91;">Open the customer booking flow</a></p></div></div></body></html>`;
}
...
function buildEmailPayload({
  ...
  return {
    ...
    senderName: "Finbar B. Elite Tutoring",
    replyTo: "hello@finbarb.com",
    studentName: cleanString(studentName),
    studentName2: cleanString(studentName2),
    subject: cleanString(emailSubject) || (kind === "booking" ? buildBookingSubject(subject) : buildReceiptSubject(subject)),
    html: buildEmailHtml({
    ...
    finHtml: buildFinHtml({
      heading: heading || (kind === "booking" ? "Booking confirmed" : "Package purchase"),
      subject,
      format,
      tier,
      totalPrice,
...
async function sendNotificationEmails({
  ...
  const resolvedKind = templateKind || kind || "booking";
  const defaults = defaultTemplates().templates[resolvedKind] || defaultTemplates().templates.booking;
  let saved = defaults;
  try {
    const state = await readTemplateState();
    saved = state.templates[resolvedKind] || defaults;
  } catch {
    // Payment emails must still send when the template editor has not been connected yet.
  }
  const variables = {
    recipientName,
    subject,
  ...
  const template = {
    subject: interpolateTemplate(saved.subject, variables),
    heading: interpolateTemplate(saved.heading, variables),
    message: interpolateTemplate(saved.message, variables),
    ctaLabel: interpolateTemplate(saved.ctaLabel, variables),
    extraNote: interpolateTemplate(saved.extraNote, variables)
  };
  const payload = buildEmailPayload({
    ...
    intro: template.message || intro,
    ctaLabel: template.ctaLabel || ctaLabel,
    detailLabel,
    extraNote: template.extraNote || extraNote,
