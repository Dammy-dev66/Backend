...
function defaultLedger() {
  return { version: 1, packages: [], receipts: [], appointments: [], actions: [], lastBackfillAt: "" };
}
...
function recordReceipt(state, record) {
  ...
  state.receipts.unshift({ id, email: clientKey(record.email), kind: cleanString(record.kind), orderID: cleanString(record.orderID), appointmentID: cleanString(record.appointmentID), sent: record.sent === true, createdAt: cleanString(record.createdAt) || now() });
...
function recordAction(state, record) {
  state.actions.unshift({ id: `${cleanString(record.type)}:${cleanString(record.appointmentID)}:${Date.now()}`, createdAt: now(), actor: "Fin", ...record });
